...
const LogsContainer = ({ profile, save, logBoolean, removeButtons, storyNum, logNum }) => {
  const {
    getLogs,
    logs,
    stories,
  ...
  console.log("Remove button: " + removeButtons)
  ...
  function None() {
    ...
    if (profile == 0 & count == 0 && (!logNum || logNum==0)) {
      return (
          ...
          <div className="edit-btns">
            ...
            {log.createdBy !== userId ? (
              <button
                ...
                onClick={() => profileFunc(log.createdBy, alias, iconName, getCountStories(log.createdBy),getCountLogs(log.createdBy) )}
              >
                <FiUser />
              </button>
  ...
  if (save) {
    ...
    return (
      <div>
        {logs?.map((log) => {
          if (profile ) {
            if (log.createdBy == (logBoolean ? logBoolean: userId)) {
              profilecount += 1;
              return (
                <Wrapper>
                  <div
