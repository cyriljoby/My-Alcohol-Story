...
import { BiMessage, BiReply, BiUserPlus} from "react-icons/bi";
...
const StoryContainer = ({ profile, save, storyBoolean, removeButtons, storyNum, logNum }) => {
  ...
  function None() {
    ...
    if (profile ==0  && count == 0 && (!storyNum || storyNum==0)) {
      return (
  ...
  function RenderReplyBox(job) {
    ...
    const profileFunc = (navigateName, alias, icon, storiesPosted, logsPosted) =>{
      
      window.location.reload(false);
     
      navigate(
        `/profile?navName=${navigateName}&alias=${alias}&icon=${icon}&storiesPosted=${storiesPosted}&logsPosted=${logsPosted}`
      );
   ...
   /*
    const profileFunc = (navigateName) =>{
      return (
      <Link to={`/profile/${navigateName}`}>Open Profile</Link>
      );
    };
   */
    const save = (e) => {
      ...
      <div>
        ...
        <div className="story-header">
          ...
          <div className="edit-btns">
            ...
            <button className="btn start-chat"onClick={() => profileFunc(job["job"].createdBy, alias, iconName, getCountStories(job["job"].createdBy),getCountLogs(job["job"].createdBy) )}>
              <FiUser />
            </button>
            ) : null}
            ...
            {job["job"].createdBy !== user_id ? (
              <button
                ...
                onClick={() => profileFunc(job["job"].createdBy)}
              >
                <BiMessage />
              </button>
            ) : null}
        ...
        <div>
          {stories?.map((job) => {
            if (profile) {
              ...
              if (job.createdBy == (storyBoolean ? storyBoolean: userId)) {
